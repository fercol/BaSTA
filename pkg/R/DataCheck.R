...
#  }   
  ...
  # 7. Categorical covariates adding to 0:
	if(ncol(object) > nt + 3){
		covType <- .FindCovType(Z)
		if (!is.null(covType$cat)) {
			sumZ <- apply(Z[, names(covType$cat)], 1, sum)
			onepos <- which(sumZ == 1)
			type7 <- which(sumZ == 0)
			if (length(type7) > 0) {
				cat("The following rows have caterogical covariates adding to 0:\n")
				print(type7)
				if (autofix[7] == 1) {
					bd <- bd[onepos, ]
					Y <- Y[onepos, ]
					idnames <- idnames[onepos]
					Z <- Z[onepos, ]
					Tm <- Tm[onepos, ]
					n <- nrow(Y)
					cat("These records have been removed from the Dataframe\n")					
...
#--------1---------2
ok <- length(c(type1, type2, type3, type4, type5, type6, type7)) == 0
  ...
  return(list(ok = ok, newData = newData, type1 = type1, type2 = type2, 
